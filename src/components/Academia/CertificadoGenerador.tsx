...
export function CertificadoGenerador() {
  const searchParams = useSearchParams()
  const cursoId = searchParams.get('curso') || '1'
  const participanteName = searchParams.get('name') || 'Participante'
  const [generando, setGenerando] = useState(false)

  const folio = `NCH-${new Date().getFullYear()}-${Math.random().toString(36).substring(7).toUpperCase()}`
  const fechaCompletacion = new Date().toLocaleDateString('es-MX', {
    year: 'numeric',
    month: 'long',
    day: 'numeric',
  })
        ...
        >
          ...
          <div className="text-center mb-12">
            ...
            <div className="border-b-2 border-nexiom-bronze py-4 mb-8">
              <p className="text-2xl font-bold text-nexiom-bronze">Capital Humano Estratégico</p>
            </div>

            <p className="text-base text-nexiom-dark leading-relaxed">
              Demostrando competencia en inteligencia organizacional, transformación digital
              y gestión estratégica del talento humano.
            </p>
          </div>
